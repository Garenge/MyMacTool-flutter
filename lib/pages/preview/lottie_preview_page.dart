...
class _LottiePreviewPageState extends State<LottiePreviewPage> {
  ...
  static const LottieFileCollector _fileCollector = LottieFileCollector();
  static const LottieFileSummaryFormatter _summaryFormatter =
      LottieFileSummaryFormatter();

  final List<LottieFileInfo> _files = <LottieFileInfo>[];
  final Set<String> _selectedPaths = <String>{};
  bool _isDraggingFiles = false;
  String? _statusText;
  String? _errorText;

  List<LottieFileInfo> get _selectedFiles {
    return _files
        .where((LottieFileInfo file) => _selectedPaths.contains(file.path))
  ...
  Future<void> _handleOpenFolder() async {
    ...
    final records = await _fileCollector.collectFromPaths(
  ...
  Future<void> _handleDropFiles(List<DropItem> items) async {
    ...
    final records = await _fileCollector.collectFromPaths(paths);
  ...
  void _replaceFiles(List<LottieFileInfo> records) {
    _files
      ..clear()
      ..addAll(records);
    _selectedPaths
      ..clear()
      ..addAll(records.map((LottieFileInfo item) => item.path));
  }

  void _mergeFiles(List<LottieFileInfo> records) {
    final nextMap = <String, LottieFileInfo>{
    ...
    _files.sort(
      (LottieFileInfo left, LottieFileInfo right) =>
  ...
  void _handleSelectAll() {
    setState(() {
      _selectedPaths
        ..clear()
        ..addAll(_files.map((LottieFileInfo file) => file.path));
  ...
  void _handleInvertSelection() {
    setState(() {
      final nextSelection = _files
          .where((LottieFileInfo file) => !_selectedPaths.contains(file.path))
          .map((LottieFileInfo file) => file.path)
          .toSet();
      _selectedPaths
        ..clear()
  ...
  Future<void> _handleCopySelectedPaths() async {
    ...
    await Clipboard.setData(
      ClipboardData(
        text: selectedFiles.map((LottieFileInfo file) => file.path).join('\n'),
      ),
    );
  ...
  Future<void> _handleCopySelectedSummary() async {
    ...
    await Clipboard.setData(
      ClipboardData(
        text: _summaryFormatter.formatSelectedSummary(selectedFiles),
      ),
    );
      ...
      children: [
        ...
        SizedBox(
          ...
          child: _LottieFileListPane(
            ...
            formatTimestamp: _summaryFormatter.formatTimestamp,
            formatFileSize: _summaryFormatter.formatFileSize,
          ),
        ),
      ],
    );
...
class _LottiePreviewPane extends StatelessWidget {
  ...
  final List<LottieFileInfo> files;
