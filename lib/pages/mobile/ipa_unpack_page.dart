...
import 'ipa_archive_extractor.dart';
import 'ipa_app_info.dart';
import 'mobileprovision_profile_diagnostics.dart';

typedef IpaDirectoryOpener = Future<void> Function(String path);

String formatIpaParseStatusText(
  IpaAppInfo? appInfo, {
  required bool didOpenOutputDirectory,
}) {
  final outputDirectoryText = didOpenOutputDirectory
      ? '已自动打开输出目录。'
      : '自动打开输出目录失败，可手动打开。';

  if (appInfo == null) {
    return '解析完成，未读取到 Info.plist，$outputDirectoryText';
  }

  if (appInfo.hasEmbeddedProfileInfo) {
    return '解析完成，已读取应用信息和 embedded.mobileprovision，$outputDirectoryText';
  }

  if (appInfo.hasEmbeddedProfile) {
    return '解析完成，已读取应用信息；embedded.mobileprovision 暂未解析成功，$outputDirectoryText';
  }

  return '解析完成，已读取应用信息，$outputDirectoryText';
}
...
class IpaUnpackPage extends StatefulWidget {
  const IpaUnpackPage({
    ...
    this.archiveExtractor = const IpaArchiveExtractor(),
    this.appInfoParser = const IpaAppInfoParser(),
  });
  ...
  final IpaDirectoryOpener? directoryOpener;
  final IpaArchiveExtracting archiveExtractor;
  final IpaAppInfoParsing appInfoParser;
...
class _IpaUnpackPageState extends State<IpaUnpackPage> {
  ...
  Future<void> _unpackIpa(String path) async {
    ...
    try {
      final outputDirectory = await widget.archiveExtractor
          .extractToTempDirectory(ipaFile);
      final appInfo = await widget.appInfoParser.parseFromExtractedDirectory(
        outputDirectory,
      );
      final didOpenOutputDirectory = await _tryOpenDirectory(
      ...
      setState(() {
        ...
        _appInfo = appInfo;
        _statusText = formatIpaParseStatusText(
          appInfo,
          didOpenOutputDirectory: didOpenOutputDirectory,
        );
        _prependRecord(
          _IpaUnpackRecord(
  ...
  Future<void> _openDirectory(String path) async {
    final directoryOpener = widget.directoryOpener;

    if (directoryOpener != null) {
      await directoryOpener(path);
      return;
    }
  ...
  Future<bool> _tryOpenDirectory(String path) async {
    try {
  ...
  bool _isIpaPath(String path) {
    return hasFileExtension(path, <String>['.ipa']);
  }
