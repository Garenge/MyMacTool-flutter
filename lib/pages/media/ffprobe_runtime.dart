...
class FfprobeRuntime {
  ...
  Future<String> installFrom(String sourcePath) async {
    ...
    final importPath =
        '$targetPath.importing.${DateTime.now().microsecondsSinceEpoch}';
    final importingFile = await source.copy(importPath);

    try {
      await _makeExecutable(importPath);

      if (!await _canRun(importPath)) {
        throw const FormatException('导入的 ffprobe 无法执行，请确认文件适用于当前 Mac。');
      }

      await _replaceManagedExecutable(importingFile, target);
    } catch (_) {
      await _deleteIfExists(importingFile);
      rethrow;
    }
  ...
  Future<void> _replaceManagedExecutable(
    File importingFile,
    File target,
  ) async {
    final backup = target.existsSync()
        ? await target.rename(
            '${target.path}.previous.${DateTime.now().microsecondsSinceEpoch}',
          )
        : null;

    try {
      await importingFile.rename(target.path);
