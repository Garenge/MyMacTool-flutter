...
- 颜色转换
  ...
  - 支持颜色预览、通道值展示、单项结果复制和设计标注模板复制
- Hash 计算
  - 支持对文本计算 MD5、SHA-1、SHA-256、SHA-512 摘要
  - 支持单项结果复制
- 图片信息
  - 支持选择或拖拽图片查看宽高、格式、文件大小、帧数
  - 支持透明通道和透明像素判断、复制图片路径和图片摘要
  - 支持最近解析记录恢复
- 音视频信息
  - 基于 `ffprobe` 解析音频和视频文件的容器、时长、大小、总码率和元数据
  - 支持展示视频流编码、分辨率、帧率、像素格式，以及音频流编码、采样率、声道和码率
  - 支持选择或拖拽文件、复制媒体摘要、复制路径和最近记录恢复
  - 支持检测 App 内置 / App 托管 / 系统的 `ffprobe`，缺失时可启用内置组件或导入本机 `ffprobe`
- JWT 解析
...
- `crypto`：Hash 摘要计算
- `image`：图片信息解析
- `ffprobe`：音视频元信息解析，优先使用 App 内置或托管运行时，也支持系统命令兜底
- `qr_flutter`：二维码生成
- `pasteboard`：剪贴板图片读取
- `zxing2`：二维码图片解析
